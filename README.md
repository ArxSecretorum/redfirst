...
| Мёртвый код | `wired` показывает число вызовов вне тестов; ноль — красный |
...
```sh
sh tests/run              # 90 случаев в dash и bash — 180 проверок
sh tests/run --self-check # ломает инструмент и требует, чтобы набор покраснел
```

Сейчас `sh tests/run` завершается кодом **1**, и это не поломка: двенадцать
случаев помечены `red?` / `green?` — «направление верное, инструмент его пока
не выполняет». Это блокеры публикации, записанные до починки. Регрессий нет:
блокеров 0, шума 0, расхождений оболочек 0.
